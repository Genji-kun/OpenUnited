...
from OUApp import app, utils, dao, send_mail
from OUApp.models import UserRole
...
def pay():
    if current_user.is_authenticated and current_user.user_role == UserRole.Staff:
        ...
        return render_template('staff/pay.html', r=reservations, check_in=check_in, check_out=check_out,
                               orderer_name=orderer_name, orderer_email=orderer_email, total=total)
...
def pay_reservation(reservation_id):
    dao.pay_reservation(reservation_id)
    return jsonify()
